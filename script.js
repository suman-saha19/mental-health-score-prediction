...
const NUMERIC_RULES = {
  age: { integer: true, min: 10, max: 100, label: 'Age' },
  avg_daily_usage_hours: { integer: false, min: 0, max: 24, label: 'Avg. daily usage' },
  daily_unlocks: { integer: true, min: 0, max: null, label: 'Daily phone unlocks' },
  study_hours: { integer: false, min: 0, max: 24, label: 'Study hours' },
  physical_activity_hours: { integer: false, min: 0, max: 24, label: 'Physical activity' },
  sleep_hours_per_night: { integer: false, min: 0, max: 24, label: 'Sleep' },
};
...
countrySelect.addEventListener('change', () => {
  if (countrySelect.value === '__other__') {
    ...
    countryHint.textContent = 'Countries outside the model\u2019s top list are grouped as "Other" for prediction.';
...
function fieldMsgEl(name) {
  const input = document.getElementById(name === 'country' ? 'country-select' : name);
  const field = input ? input.closest('.field') : document.getElementById('country-field');
...
form.querySelectorAll('input, select').forEach((el) => {
  el.addEventListener('input', () => {
    if (el.id && el.id !== 'country-select') clearFieldError(el.id);
  });
});
...
function updateHoursAdvisory() {
  ...
  if (sum > 24) {
    ...
    hoursAdvisory.textContent = `Heads up: usage, study, activity, and sleep hours add up to ${sum.toFixed(1)} — more than a 24-hour day. The model will still run, but you may want to double-check these numbers.`;
  } else {
...
const GAUGE = { size: 220, stroke: 14 };
...
function renderGauge(score, maxScore = 10) {
  ...
  value.setAttribute('stroke-dashoffset', String(arcLength * (1 - fraction)));
  ...
  document.getElementById('gauge-svg').setAttribute('aria-label', `Predicted score ${clamped.toFixed(2)} out of ${maxScore}`);
}
...
function scoreBand(score) {
  if (score < 3.34) return { label: 'Lower range', cls: 'low' };
  if (score < 6.67) return { label: 'Moderate range', cls: 'mid' };
  return { label: 'Higher range', cls: 'high' };
}
...
async function predict(payload) {
  ...
  try {
    ...
    if (response.status === 422) {
      ...
      const fieldErrors = body ? parseValidationDetail(body) : {};
      Object.entries(fieldErrors).forEach(([field, message]) => setFieldError(field, message));
      showError('validation', 'The server rejected some of the submitted values.');
      return;
    }
    ...
    if (!response.ok) {
      ...
      showError('server', detail || `The server responded with an error (${response.status}).`);
      return;
    }
    ...
    showError('network', `Couldn't reach the prediction server at ${API_BASE_URL}. Make sure the FastAPI backend is running and reachable.`);
  } finally {
...
function runValidation() {
  ...
  if (countryError) {
    ...
    if (!firstInvalid) firstInvalid = countrySelect.value === '__other__' || countryCustomRow.hidden === false ? countryCustomInput : countrySelect;
  }
...
retryBtn.addEventListener('click', () => {
  if (lastPayload) predict(lastPayload);
});
